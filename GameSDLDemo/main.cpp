...
int main(int arc, char*argv[])
{
  ...
  if (!ret) return 0;
  //dichden
  ThreatObject*p_dich = new ThreatObject;
  p_dich->SetRect(SCREEN_WIDTH-250, 150);
  p_dich->LoadImg("dich.png");
  //Make ThreatObject
  ...
  while (!is_quit) 
  {
    ...
    else
    {
	  ...
	  p_dich->Show(g_screen);
	  bool is_col2 = SDLCommonFunc::CheckCollision(plane_object.GetRect(), p_dich->GetRect());
	  if (is_col2)
	  {
		  if (MessageBox(NULL, L"YOU WIN!", L"GAME BAN MAY BAY", MB_OK) == IDOK)
		  {
			  delete[] p_threats;
			  SDLCommonFunc::CleanUp();
			  SDL_Quit();
